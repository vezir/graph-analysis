...
CREATE (s)-[:`FLIGHT` {distance:toInteger(line.DISTANCE),air_time:line.AIR_TIME, fl_date:line.FL_DATE}]->(t)
...
MERGE (n)-[to:TO]->(n)
  ON CREATE SET to.air_time = line.AIR_TIME, 
                to.distance = toInteger(line.DISTANCE)
...
* 1 or 2 hops
MATCH (d:Airport)-[:FLIGHT*1..2]-(s:Airport) where d.airport_name='Los Angeles International' Return Distinct s.airport_name order by s.airport_name
...
match (n:Airport)-[w:FLIGHT]-() return w limit 10

-{"air_time":"92.00","distance":"692.00","fl_date":"01.01.2020"} -
+¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦+
-{"air_time":"55.00","distance":"425.00","fl_date":"01.01.2020"} -
+¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦+
-{"air_time":"79.00","distance":"425.00","fl_date":"01.01.2020"} -
+¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦+

* Enable multi statement queries
:config "enableMultiStatementMode":true

MATCH (c:Airport)-[:FLIGHT]->()
WITH c, count(*) AS num
RETURN min(num) AS min, max(num) AS max, avg(num) AS avg_interactions, stdev(num) AS stdev

* required memory estimation
CALL gds.graph.create.estimate('Airport', 'FLIGHT') YIELD nodeCount, relationshipCount, requiredMemory

*Create the graph !
CALL gds.graph.create('got-interactions', 'Airport', 'FLIGHT')
...
CALL gds.graph.create.cypher(
  ...
  'MATCH (n:Airport) RETURN id(n) AS id',
  'MATCH (s:Airport)-[f:FLIGHT]->(t:Airport) RETURN id(s) AS source, id(t) AS target, f.weight AS weight'
)
...
RETURN gds.util.asNode(nodeId).airport_name AS name, score
...
calismadan sonra graf bellekten silinir !
...
CALL gds.graph.create('got-interactions', 'Airport', {
  FLIGHT: {
    orientation: 'UNDIRECTED'
  }
})
...
CALL gds.graph.create.cypher(
  ...
  'MATCH (n:Airport) RETURN id(n) AS id',
  'MATCH (p1:Airport)-[:MEMBER_OF_CULTURE]->(c:Culture)<-[:MEMBER_OF_CULTURE]-(p2:Person) RETURN id(p1) AS source, id(p2) AS target'
)
